...
const features = {
     ...
     */
    async debitRequest(phone, amount, firstname, lastname, email, direct, stateData){
        channel = await f.getSocketChannel()
        config.contact = channel
        rtime.init(config)

        if (channel) {
            this._debitResponse = await f.request(phone, amount, channel, firstname, lastname, email, direct, stateData)
        }
